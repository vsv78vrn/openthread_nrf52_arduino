...
oc_connectivity_get_endpoints(size_t device)
{
  ...
  address = otIp6GetUnicastAddresses(otrGetInstance());
  while (address) {
...
oc_send_buffer(oc_message_t *message)
{
  ...
  otMessage *ot_message = otUdpNewMessage(otrGetInstance(), &settings);
...
oc_connectivity_init(size_t device)
{
  ...
  if (otIp6SubscribeMulticastAddress(otrGetInstance(), &maddr) != OT_ERROR_NONE) {
    OC_ERR("Can't subscribe mcast address");
    return -1;
  }

  if (otUdpOpen(otrGetInstance(), &unicast_socket,
  ...
  if (otUdpOpen(otrGetInstance(), &multicast_socket,
                udp_receive_cbk, NULL) != OT_ERROR_NONE) {
